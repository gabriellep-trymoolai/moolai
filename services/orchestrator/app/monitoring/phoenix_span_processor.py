...
class PhoenixLLMSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span: ReadableSpan, parent_context: Optional[object] = None) -> None:
        ...
        if hasattr(span, 'set_attribute'):
            ...
            if self._is_llm_span(span.name, getattr(span, 'attributes', {})):
                try:
                    # Add attributes during span creation when it's still writable
                    span.set_attribute("llm.system", self._extract_llm_system(getattr(span, 'attributes', {})))
                    span.set_attribute("phoenix.span_type", "LLM")
                    
                    # Extract model name
                    model_name = self._extract_model_name(getattr(span, 'attributes', {}))
                    if model_name:
                    ...
                    # Add cache-related attributes if available
                    attributes = getattr(span, 'attributes', {})
                    if 'cache_hit' in str(attributes) or 'from_cache' in str(attributes):
                        # Extract cache information from span context
                        span.set_attribute("cache.hit", attributes.get('from_cache', False))
                        if 'cache_similarity' in str(attributes):
                            span.set_attribute("cache.similarity", attributes.get('cache_similarity', 0.0))
                except Exception:
